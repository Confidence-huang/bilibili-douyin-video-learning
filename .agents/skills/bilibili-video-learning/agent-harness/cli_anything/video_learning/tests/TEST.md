...
- 安装入口：`cli-anything-video-learning` 1.1.0
...
- 29 个核心单测全部通过：安全 yt-dlp 参数、严格分 P、JSON3/VTT、Cookie 全加密保护、默认省略全文、不可信正文规则、CLI/后端错误脱敏、Cookie 授权状态、原子写入、来源身份、抖音缓存身份和音频清理。
- 11 个安装态 subprocess 测试通过：包含 1.1.0 版本、纯 JSON、非法分 P、字幕/笔记原子输出、来源身份与真实 doctor。
- 当前总计 40 个离线/安装态测试；默认套件不联网、不下载媒体、不读取浏览器 Cookie，也不启动 GPU ASR。
